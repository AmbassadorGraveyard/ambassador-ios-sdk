...
- (void)testDevPusherSessionSubscribe {
    ...
    [[AMBAmbassadorNetworkManager sharedInstance] sendNetworkObject:nil url:[AMBAmbassadorNetworkManager pusherSessionSubscribeUrl] universalToken:self.devToken universalID:self.devID completion:^(NSData *d, NSURLResponse *r, NSError *e) {
        if (e) {
            XCTFail(@"%@", e);
        } else {
