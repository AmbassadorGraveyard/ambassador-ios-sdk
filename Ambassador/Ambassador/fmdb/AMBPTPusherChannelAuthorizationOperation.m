...
@interface AMBPTPusherChannelAuthorizationOperation ()
@property (nonatomic, strong, readwrite) NSDictionary *authorizationData;
@property (nonatomic, readwrite) NSError *error;
@end

@implementation AMBPTPusherChannelAuthorizationOperation
...
- (void)finish
{
  ...
  if (self.connectionError) {
    self.error = [NSError errorWithDomain:AMBPTPusherErrorDomain code:PTPusherChannelAuthorizationConnectionError userInfo:@{NSUnderlyingErrorKey: self.connectionError}];
  }
  else {
    _authorized = YES;
    ...
    if (_authorized) {
      _authorizationData = [[AMBPTJSON JSONParser] objectFromJSONData:responseData];
      ...
      if (![_authorizationData isKindOfClass:[NSDictionary class]]) {
        ...
        self.error = [NSError errorWithDomain:AMBPTPusherErrorDomain code:PTPusherChannelAuthorizationBadResponseError userInfo:userInfo];
...
@implementation AMBPTPusherChannelAuthorizationBypassOperation {
